...
|---|---|---|---|---|
| Causal reasoning | Correlations reported as effects; "the numbers went up" treated as proof | Confounding acknowledged; some adjustment attempted case by case | Counterfactual stated explicitly; a recognised design chosen for each claim | Multiple designs triangulated; assumptions tested and effects understood as local or general |
| Study design & commissioning | Evaluation bought as a retrospective report after rollout | Comparison groups chosen but not pre-planned | Designs planned before data collection; analysis plans pre-registered | Operational rollouts routinely randomised or staggered to generate evidence |
| Data & missingness | Incomplete records dropped silently; data shape ignored | Missingness noted; some appropriate models for costs and counts | Missing-data handling stated and tested; models matched to data shape | Sensitivity to missingness and specification reported as standard |
| Appraisal capability | Results accepted on authority or peer-review label | Some staff can question a headline estimate | Team routinely asks what must be true for a claim to be causal | Independent appraisal embedded; standards of proof agreed before decisions |
| Use in decisions | Evidence used selectively to support chosen course | Evidence weighed but double standards uncontrolled | Standard of proof and external validity considered before acting | Pre-committed evidence standards; results published regardless of direction |
