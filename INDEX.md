# Index

Concepts and frameworks mapped to the chapters that cover them. Numbers are **chapter numbers, not pages** — e.g. `2.1` is Chapter 2.1 — Economic Evaluation. Each entry lists the home chapter plus every chapter that materially covers the concept, ascending and de-duplicated.

Entries are alphabetical within each letter section. This file is populated as chapters are written; empty letter sections are placeholders.

---

## A

**"A QALY is a QALY is a QALY"** — 3.5
**Accountability for reasonableness** — 3.3, 3.4
**Accountable care organizations** — 3.8
**Activities of daily living** — 3.9
**Active travel** — 4.5
**Actuarial science** — 3.8
**Adaptation (hedonic adaptation)** — 3.5
**Adverse events and medical error** — 3.12
**Adverse selection** — 1.3, 3.1, 3.8, 3.9
**Advance market commitment** — 3.6, 4.2
**Affordability versus value** — 2.5
**Agency and the principal–agent problem** — 1.1, 1.2, 1.3, 3.1, 3.11
**Ageing and dependency** — 3.9
**Air pollution** — 4.5
**Algorithmic bias** — 3.4, 4.4
**Allocative efficiency** — 1.1, 1.4, 3.4
**Antimicrobial resistance** — 1.3, 3.2, 3.6, 4.2
**Arrow and the founding of health economics** — 1.1, 1.3
**Artificial intelligence in health care** — 4.3, 4.4
**Automation and the health workforce** — 3.7, 4.4
**Automation bias** — 4.4

## B

**Baumol effect (cost disease)** — 1.3, 3.1
**Behavioural economics** — 1.2, 4.1
**Benefit-package design** — 3.3, 3.8
**Beveridge model** — 3.1
**Biosimilars** — 2.4
**Bismarck model (social health insurance)** — 3.1
**Block, cost-and-volume, and cost-per-case contracts** — 3.11
**Bounded rationality** — 4.1
**Budget impact analysis** — 2.5
**Burnout and retention** — 3.7
**Business case for safety** — 3.12

## C

**Capability approach** — 2.1, 3.5, 3.9, 3.10
**Capacity, occupancy, and deliberate slack** — 1.4, 3.13
**Capitation** — 1.2, 1.3, 3.1, 3.8
**Carbon footprint of care** — 4.5
**Causal inference** — 2.3, 2.6
**Choice architecture and nudges** — 3.2, 4.1
**Climate change as a driver of health cost** — 4.5
**Clinical decision support systems** — 4.4
**Co-benefits of climate action** — 4.5
**Co-dependent technology** — 4.2
**Coalition for Epidemic Preparedness Innovations (CEPI)** — 3.13
**Cochrane and systematic-review methods** — 2.6
**Commercial determinants of health** — 1.5
**Commissioning and the commissioning cycle** — 3.11
**Community rating** — 3.8
**Companion diagnostic** — 4.2
**Comparator, choice of** — 2.1, 2.2, 2.4, 2.6, 4.4
**Compensating differentials** — 3.7
**Compulsory licensing** — 3.6
**Concentration index and curve** — 3.4
**Concept drift** — 4.4
**Confounding** — 1.5, 2.3
**Contract theory and incomplete contracts** — 3.11
**Cost containment** — 3.1, 3.3
**Cost of delay** — 2.5
**Cost of poor quality** — 3.12
**Cost of production (fixed, variable, marginal, average)** — 1.4
**Cost–benefit analysis** — 2.1
**Cost-effectiveness analysis** — 2.1, 2.4, 2.5, 3.2, 3.4, 3.6
**Cost-effectiveness threshold** — 2.1, 2.4, 2.5, 3.3, 3.6
**Cost-sharing and co-payments** — 1.2, 1.3, 3.1, 3.3, 3.8
**Cost-shifting across budgets** — 3.9, 3.10, 3.12
**Cost–utility analysis** — 2.1, 2.4
**Creative destruction** — 4.2

## D

**DALY (disability-adjusted life year)** — 1.1, 2.1, 3.6
**Data as an asset and data governance** — 4.3, 4.4
**Decision analysis** — 2.2
**Decision trees** — 2.2
**Defaults and the default effect** — 1.3, 3.2, 4.1
**Deinstitutionalization** — 3.10
**Demand for health and derived demand** — 1.1, 1.2, 3.7
**Dementia** — 3.9
**Dependency ratio** — 3.9
**Deskilling** — 4.4
**Determinants of health** — 1.1, 1.5
**Diagnosis-related groups (DRGs)** — 3.1
**Difference-in-differences** — 2.3, 3.2
**Diffusion of innovations** — 4.2
**Digital divide and digital exclusion** — 3.4, 4.3
**Digital health tools** — 4.3, 4.4
**Diminishing returns and returns to scale** — 1.4
**Discounting** — 1.5, 2.1, 2.2, 2.5, 3.2, 3.9, 3.10, 4.3, 4.5
**Discrete-event simulation** — 2.2
**Disinvestment** — 1.1, 2.1, 3.1, 3.3, 4.5
**Distributional cost-effectiveness analysis** — 2.1, 3.4
**Distributive justice** — 3.3, 3.4
**Division of labour and task-shifting** — 3.7
**Donabedian model** — 3.12

## E

**Econometric methods** — 2.3
**Economic efficiency (technical and allocative)** — 1.1, 1.4, 3.4
**Economic evaluation** — 1.1, 2.1, 2.2, 2.4, 2.5, 4.2, 4.3, 4.4
**Economies of scale and scope** — 1.4
**Eligibility thresholds and cliff-edges** — 3.9
**Environmental determinants of health** — 1.5, 4.5
**EQ-5D** — 2.1, 3.5, 3.10
**Equity** — 1.1, 1.5, 2.1, 2.5, 3.2, 3.3, 3.4, 3.6, 3.7, 3.8, 3.9, 3.10, 4.1, 4.3, 4.4, 4.5
**Equity weights** — 3.4, 3.5
**Equity–efficiency trade-off** — 1.1, 2.1, 3.4
**Evidence-based policy** — 3.2
**Evidence hierarchy and GRADE** — 2.6
**Evidence standards for digital and AI tools** — 4.3, 4.4
**Evidence synthesis and meta-analysis** — 2.1, 2.2, 2.6
**Evergreening** — 2.4
**Expected value and expected utility** — 3.13
**Experience rating** — 3.8
**External reference pricing** — 2.4
**Externalities** — 1.1, 1.3, 1.5, 3.2, 3.6, 3.13, 4.2, 4.5
**Extra-welfarism** — 3.5

## F

**Fair innings argument** — 3.4
**Fee-for-service** — 1.2, 1.3, 3.1
**Financial headroom** — 2.5
**Financial protection and catastrophic spending** — 3.1, 3.4, 3.6, 3.8
**Financing models** — 3.1, 3.6, 3.9
**Fiscal space** — 3.1
**Forest plots** — 2.6
**Framework Convention on Tobacco Control** — 3.2
**Framing** — 4.1
**Fundamental cause theory** — 1.5
**Funnel plots and publication bias** — 2.6

## G

**Gavi** — 3.6, 4.2
**Generic and condition-specific measures** — 2.1
**Generic drugs** — 2.4
**Gini coefficient** — 3.4
**Global budgets** — 3.1
**Global catastrophic risk** — 3.13
**Global Fund** — 3.6
**Global mental health** — 3.10
**Global public goods** — 1.3, 3.6, 3.13
**GRADE and certainty of evidence** — 2.6
**Grey literature** — 2.6
**Grossman model and health as capital** — 1.1, 1.2, 4.1

## H

**Headroom analysis (early HTA)** — 4.2
**Health care quality** — 3.12
**Health econometrics** — 2.3
**Health in All Policies** — 1.5, 3.2
**Health inequalities and the social gradient** — 1.5, 3.4
**Health insurance and demand for insurance** — 1.2, 1.3, 3.1, 3.8
**Health policy instruments** — 1.3, 3.2
**Health spending growth (cost pressure)** — 1.3, 3.1
**Health systems** — 3.1, 3.6
**Health technology assessment (HTA)** — 1.1, 2.1, 2.2, 2.4, 2.5, 2.6, 3.2, 3.3, 3.11, 4.2, 4.3, 4.4
**Health–social-care boundary** — 3.9
**Health-worker migration (brain drain)** — 3.6, 3.7
**Healthcare-associated infection** — 3.12
**Heterogeneity** — 2.6
**Heuristics and biases** — 4.1
**High-risk strategy (prevention)** — 3.2
**Horizontal and vertical equity** — 3.4
**Human capital** — 1.1, 1.2, 3.7
**Human Development Index** — 3.5

## I

**Iatrogenesis** — 3.12
**ICECAP measures** — 3.5, 3.9
**Incremental cost-effectiveness ratio (ICER)** — 2.1, 2.2, 2.4, 2.5, 3.3
**Income effects on demand** — 1.2
**Inducement prizes** — 4.2
**Informal and unpaid care** — 3.9, 3.10
**Information asymmetry** — 1.1, 1.3, 3.7, 3.11
**Innovation incentives (push and pull)** — 4.2
**Instrumental variables** — 2.3
**Insurance design and risk protection** — 3.8
**Integrated care** — 3.9, 3.11
**Integration of health and social care** — 3.9
**Intention-to-treat analysis** — 2.3
**Interoperability and open standards (FHIR)** — 4.6
**Inverse care law** — 3.4, 4.3

## J

## K

**Kakwani index** — 3.4

## L

**Labour markets in health** — 3.7
**Levels of prevention (primary, secondary, tertiary)** — 3.2
**Legacy systems and modernization** — 4.6
**Libertarian paternalism** — 4.1
**Life-course approach** — 1.5
**Life-cycle assessment** — 4.5
**Living reviews** — 2.6
**Long-term care financing** — 3.9
**Loss aversion** — 4.1

## M

**Machine learning** — 4.4
**Make-or-buy decision** — 3.11, 4.6
**Managed care** — 3.8, 3.11
**Managed entry agreements** — 2.2, 2.4, 2.5
**Mapping (cross-walking)** — 2.1
**Marginal analysis** — 1.1, 1.4, 3.3
**Marginal contribution of medical care** — 1.5
**Market failure** — 1.1, 1.3, 3.2, 3.7, 4.2
**Market structure and competition** — 1.4
**Markov models** — 2.2
**Marmot and the Whitehall studies** — 1.5, 3.4
**Means testing** — 3.9
**Medical education and the training pipeline** — 3.7
**Medical tourism** — 3.6
**Mental health economics** — 3.10
**Meta-analysis** — 2.6
**mHealth and eHealth** — 4.3
**Microsimulation** — 2.2
**Missing data** — 2.3
**Modelling (decision-analytic)** — 2.1, 2.2
**Monopoly** — 1.3, 1.4, 2.4
**Monopsony and wage-setting** — 3.7
**Monte Carlo simulation** — 2.2
**Moral hazard** — 1.2, 1.3, 3.1, 3.8

## N

**Natural experiments** — 2.3, 3.2
**Natural monopoly** — 1.4
**Net zero and decarbonization** — 4.5
**Network effects and vendor lock-in** — 4.3, 4.4, 4.6
**Network meta-analysis** — 2.6
**Never events** — 3.12
**NICE (National Institute for Health and Care Excellence)** — 1.1, 2.1, 2.2, 2.4, 2.5, 2.6, 3.2, 3.3, 3.11, 4.3, 4.4

## O

**Occupational licensing and scope of practice** — 3.7
**Open-source software** — 4.6
**Opportunity cost** — 1.1, 2.1, 2.4, 2.5, 3.3, 3.13, 4.3, 4.5
**Option value and real options** — 3.13
**Oregon Health Plan** — 3.3
**Orphan drugs** — 2.4, 4.2
**Outcome-based contracts** — 3.11
**Out-of-pocket payment and maximums** — 3.1, 3.4, 3.6, 3.8

## P

**Pandemic preparedness** — 3.6, 3.13
**Pandemics as global externalities** — 3.6, 3.13
**Panel data methods** — 2.3
**Panic-and-neglect cycle** — 3.13
**Parity of esteem** — 3.10
**Passive versus strategic purchasing** — 3.11
**Patents** — 2.4, 3.6, 4.2
**Patient safety** — 3.12
**Patient values versus public values** — 3.5
**Patient-reported outcome measures (PROMs)** — 2.1, 4.3
**Pay-for-performance** — 3.12
**Perspective (analytic)** — 2.1, 2.2, 3.6, 3.9, 3.10, 4.5
**Pharmacoeconomics** — 2.4
**Phased and managed adoption** — 2.5
**Pigovian tax** — 3.2
**Planetary health** — 4.5
**Plumbing diagram (Williams)** — 1.1, 1.2, 1.5
**Pooling and prepayment** — 1.3, 3.1, 3.8, 3.9
**Population health** — 1.5
**Population strategy (prevention)** — 3.2
**Precautionary principle** — 3.13
**Precision medicine (stratified / personalized)** — 4.2
**Present bias** — 3.2, 4.1
**Presenteeism and productivity loss** — 3.10
**Prevention economics** — 1.2, 1.5, 2.1, 3.2, 3.10, 4.3
**Prevention paradox** — 1.5, 3.2
**Price discrimination** — 2.4
**Price elasticity of demand** — 1.2
**Prioritarianism** — 3.4
**Priority-setting** — 1.1, 3.3, 3.6, 3.11
**Prior authorization and utilization management** — 3.8
**PRISMA reporting** — 2.6
**Procedural justice** — 3.3, 3.4
**Procurement and public procurement** — 3.11
**Production function and inputs** — 1.4
**Programme budgeting and marginal analysis** — 1.1, 3.3
**Progressive and regressive financing** — 3.1, 3.2, 3.4
**Proportionate universalism** — 1.5
**Provider objectives and behaviour** — 1.4
**Provider payment mechanisms** — 1.2, 1.3, 3.1, 3.8, 3.11, 3.12
**Public goods** — 1.3, 3.6, 3.13, 4.2
**Public health** — 1.5, 3.2
**Publication bias** — 2.6
**Purchaser–provider split** — 3.11

## Q

**QALY (quality-adjusted life year)** — 1.1, 2.1, 2.2, 2.4, 2.5, 3.3, 3.4, 3.5, 3.10
**Quality measurement and the Donabedian model** — 3.12

## R

**RAND Health Insurance Experiment** — 1.3, 3.1, 3.8
**Random-effects and fixed-effect models** — 2.6
**Randomized controlled trials** — 2.2, 2.3, 2.6, 4.1
**Rationing** — 1.1, 2.5, 3.1, 3.3, 3.4, 3.6
**Rawlsian maximin ("just health")** — 3.4
**Real-world evidence** — 2.4, 4.3, 4.4
**Reference case** — 2.1, 2.4
**Regression discontinuity design** — 2.3
**Regulation** — 1.3, 3.2, 4.4
**Regulatory capture** — 3.2
**Reinsurance** — 3.8
**Remote patient monitoring** — 4.3
**Research and development economics** — 2.4, 4.2
**Response shift** — 3.5
**Return on investment (ROI and SROI)** — 2.1, 3.2, 4.3
**Risk adjustment and risk equalization** — 1.3, 3.1, 3.8
**Risk pooling** — 1.3, 3.1, 3.8, 3.9
**Root cause analysis** — 3.12
**Rule of Rescue** — 2.4, 3.3

## S

**Scarcity** — 1.1, 3.3
**Selection bias** — 2.3
**Selective contracting** — 3.11
**Sensitivity and uncertainty analysis** — 2.1, 2.2
**Sin taxes and consumption taxes** — 3.2
**Single-payer systems** — 3.1
**Site reliability engineering** — 4.6
**Skill mix and task-shifting** — 3.7
**Social care economics** — 3.9
**Social cost of carbon** — 4.5
**Social determinants of health** — 1.5, 3.4, 4.5
**Social gradient in health** — 1.5, 3.4
**Social impact bonds** — 3.11
**Software maintenance** — 4.6
**Strategic purchasing** — 3.11
**Subjective wellbeing** — 3.5
**Sufficientarianism** — 3.4
**Sugary drink taxes** — 3.2
**Supplier-induced demand** — 1.2, 1.3, 3.1
**Supply of healthcare** — 1.4
**Surge capacity** — 3.13
**Sustainable and low-carbon care** — 4.5
**Systematic reviews** — 2.6

## T

**Tail risk and catastrophe** — 3.13
**Technical debt** — 4.6
**Telehealth and telemedicine** — 4.3
**Time preference** — 1.2
**Time trade-off** — 2.1
**Total cost of ownership** — 4.6
**Treatment gap** — 3.10
**TRIPS Agreement** — 3.6
**Two-tier healthcare** — 3.1, 3.6

## U

**Uncertainty (parameter, structural, heterogeneity)** — 2.1, 2.2, 2.6, 3.13, 4.2
**Universal health coverage** — 3.1, 3.4, 3.6, 3.8
**Uptake curves and eligible populations** — 2.5
**Utilitarianism** — 3.4, 3.5

## V

**Validation of models** — 2.2
**Value of a statistical life** — 3.13
**Value of information** — 2.2, 4.2
**Value-based health care** — 3.11
**Value-based insurance design** — 3.8
**Value-based pricing** — 2.4
**Value-based purchasing** — 3.11, 3.12
**Visual analogue scale and standard gamble** — 2.1

## W

**Waiting lists** — 3.1, 3.3
**Wearables and remote monitoring** — 4.3
**Welfarism and extra-welfarism** — 3.5
**Wellbeing measurement** — 3.5, 3.9
**Whitehall studies and the social gradient** — 1.5, 3.4
**Willingness to pay** — 2.1
**Workforce and human resources for health** — 1.4, 3.1, 3.6, 3.7, 4.4

## X

## Y

## Z
